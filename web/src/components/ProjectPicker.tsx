...
export function useProjectSelection(organization: Organization | undefined) {
  ...
  const project = selected ? projects.data?.projects.find(item => item.id === selected) : projects.data?.projects[0];
  return { projects, project, select: (id: string) => setParams({ organization: organization?.id ?? '', project: id }) };
}
export function ProjectPicker({ selection }: { selection: ReturnType<typeof useProjectSelection> }) {
  return <><ErrorNotice error={selection.projects.error} retry={selection.projects.reload} />
    ...
    {!!selection.projects.data?.projects.length && <label className="project-picker">Project<select value={selection.project?.id ?? ''} onChange={e => selection.select(e.target.value)}>{!selection.project && <option value="" disabled>Select an available project</option>}{selection.projects.data.projects.map(project => <option value={project.id} key={project.id}>{project.name}{project.archived_at ? ' · archived' : ''}</option>)}</select></label>}
  </>;
}
