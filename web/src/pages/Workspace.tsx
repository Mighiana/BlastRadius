...
import { jobError, jobSchema, mutate, projectSchema, request, type Job, type Organization } from '../api';
...
const historySchema = z.object({ analyses: z.array(jobSchema) });
...
function WorkspaceContent() {
  ...
  const org = session?.organizations.find(item => item.id === orgId) ?? session?.organizations[0];
  ...
  const project = projects.data?.projects.find(item => item.id === projectId) ?? projects.data?.projects[0];
  const [historyLimit, setHistoryLimit] = useState(50);
  const [historyPage, setHistoryPage] = useState(0);
  const history = useResource(project ? `/api/projects/${encodeURIComponent(project.id)}/analyses?limit=${historyLimit}&offset=${historyPage * historyLimit}` : null, historySchema);
  ...
  return <>
    ...
    <div className="workspace-layout"><aside className="project-sidebar panel">
      ...
      <div className="project-list">{projects.data?.projects.map(item => <button key={item.id} aria-pressed={item.id === project?.id} onClick={() => selectProject(item.id)}>{item.name}<ArrowRight size={15} aria-hidden="true" /></button>)}</div>
      {projects.data?.projects.length === 0 && <p>No projects yet. Create your first to get started.</p>}
      {canWrite && org && <form className="project-form" onSubmit={event => { void createProject(event); }}><label>Project name<input placeholder="e.g. payments-production" required maxLength={100} value={projectName} onChange={event => setProjectName(event.target.value)} /></label><button className="button secondary" disabled={busy}><FolderPlus size={16} aria-hidden="true" />Create project</button></form>}
      ...
      <div className="workspace-main">
        ...
        {project && <>
          {!historyOnly && canWrite && <AnalysisForm key={`${org?.id}-${project.id}`} projectId={project.id} onSubmitted={submitted} />}
          ...
          <section className="panel history-panel"><div className="panel-heading"><div><p className="eyebrow">{project.name}</p><h2>Analysis history</h2></div><button className="button secondary" onClick={history.reload}><RefreshCw size={15} aria-hidden="true" />Refresh</button></div>
            ...
            {!history.loading && !history.data?.analyses.length && !history.error && <p className="muted">No analyses on this page. Upload Terraform to run your first comparison.</p>}
            <div className="history-list">{history.data?.analyses.map(job => <article key={job.id} className={job.id === analysisId ? 'selected' : ''}>
              <button className="history-open" onClick={() => setParams({ project: project.id, analysis: job.id })}><span><strong>{job.base_label} → {job.candidate_label}</strong><small>{new Date(job.created_at * 1000).toLocaleString()}</small></span>
